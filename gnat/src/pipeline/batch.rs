...
impl BatchProcessor {
    pub fn new(
        ...
        if use_motherduck {
            ...
            md_conn.execute_batch(MD_FLOW_TABLE).expect("execute_batch");
    ...
    fn process_motherduck(&mut self, file_list: &Vec<String>) -> Result<(), Error> {
...
impl FileProcessor for BatchProcessor {
    ...
    fn process(&mut self, file_list: &Vec<String>, schema_version: FileType) -> Result<(), Error> {
        if self.use_motherduck {
            // Propagate error if process_motherduck fails
            self.process_motherduck(file_list)?;
        } else {
            ...
            conn.execute_batch(&sql_command).map_err(|e| Error::new(std::io::ErrorKind::Other, format!("DuckDB error: {}", e)))?;

            fs::rename(&tmp_filename, &final_filename).map_err(|e| Error::new(std::io::ErrorKind::Other, format!("File rename error: {}", e)))?;
        }
        Ok(())
    }
}
