...
if [ -z "${GNAT_OPTIONS}" ]; then
    GNAT_OPTIONS="--entropy --ndpi --verbose  --mac --max-payload=2048 --flow-stats --no-template-metadata --no-element-metadata --no-tombstone --active-timeout 180 --idle-timeout 60 --out ${GNAT_OUTPUT_DIR}/${GNAT_OBSERVATION_TAG} --lock"
